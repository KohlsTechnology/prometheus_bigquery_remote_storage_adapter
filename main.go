...
func serve(logger slog.Logger, addr string, writers []writer, readers []reader) {
	...
	http.HandleFunc("/write", func(w http.ResponseWriter, r *http.Request) {
		logger.Debug("Request", slog.Any("method", r.Method), slog.Any("path", r.URL.Path))

		begin := time.Now()
		compressed, err := io.ReadAll(r.Body)
		if err != nil {
			logger.Error("Read error", slog.Any("err", err.Error()))
		...
		if err != nil {
			logger.Error("Decode error", slog.Any("err", err.Error()))
		...
		if err := proto.Unmarshal(reqBuf, &req); err != nil {
			logger.Error("Unmarshal error", slog.Any("err", err.Error()))
			http.Error(w, err.Error(), http.StatusBadRequest)
			writeErrors.Inc()
			return
		}
		...
		logger.Debug("/write", slog.Any("duration", duration))
	})
	...
	http.HandleFunc("/read", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.Warn("Error executing query", slog.Any("query", req), slog.Any("storage", reader.Name()), slog.Any("err", err))
			http.Error(w, err.Error(), http.StatusInternalServerError)
			readErrors.Inc()
			return
		}
		...
		if _, err := w.Write(compressed); err != nil {
			logger.Warn("Error writing response", slog.Any("storage", reader.Name()), slog.Any("err", err))
			readErrors.Inc()
		}
		duration := time.Since(begin).Seconds()
		readProcessingDuration.WithLabelValues(writers[0].Name()).Observe(duration)
		logger.Debug("/read", slog.Any("duration", duration))
	})

	if err := srv.ListenAndServe(); err != http.ErrServerClosed {
		logger.Error("Failed to listen", slog.Any("addr", addr), slog.Any("err", err))
		os.Exit(1)
	}
...
func sendSamples(logger slog.Logger, w writer, timeseries []*prompb.TimeSeries) {
	...
	duration := time.Since(begin).Seconds()
	if err != nil {
		logger.Warn("Error sending samples to remote storage", slog.Any("err", err), slog.Any("storage", w.Name()), slog.Any("num_samples", len(timeseries)))
